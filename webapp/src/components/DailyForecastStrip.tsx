...
export function DailyForecastStrip({
  ...
  const daysData = useMemo<DayForecastItem[]>(() => {
    // SOURCE PRIORITY: explicitly-passed hours (per-station model series, with a
    // live anchor) win over the city-wide forecast — the station forecast module
    // feeds this strip, so the daily outlook must follow the selected station.
    const rawHours = hours.length > 0 ? hours : forecast?.forecast_hours ?? [];
