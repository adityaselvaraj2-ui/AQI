...
export function CardStack<T extends CardStackItem>({
  ...
  // Constrain cardWidth so the entire fan (2 * maxOffset * cardSpacing + cardWidth) fits within container
  const maxAllowableCardWidth = Math.min(cardWidth, Math.max(280, containerWidth - 40));
  const effectiveCardWidth = maxAllowableCardWidth;
  const effectiveCardHeight = Math.round(cardHeight * (effectiveCardWidth / cardWidth));

  const rawCardSpacing = Math.round(effectiveCardWidth * (1 - overlap));
  // Guarantee outer wings stay within container boundaries
  const maxAvailableHalfWidth = (containerWidth - effectiveCardWidth) / 2;
  const cardSpacing = maxOffset > 0 
    ? Math.max(10, Math.min(rawCardSpacing, Math.floor(maxAvailableHalfWidth / maxOffset)))
    : rawCardSpacing;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ height: Math.max(340, effectiveCardHeight + 70) }}
        tabIndex={0}
        onKeyDown={onKeyDown}
      >
            ...
            {items.map((item, i) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    "absolute bottom-0 rounded-[22px]",
