...
export function HealthCareAssistantPage({
  ...
  // Optional per-user key (own Gemini/Groq quota). The default server key
  // lives in the backend .env and is injected by the proxy — never here.
  const [apiKey, setApiKey] = useState<string>(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("delhi_aqi_groq_api_key") || "";
    }
    return "";
  });
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.8rem", flexWrap: "wrap" }}>
            ...
            >
              <KeyRound size={12} />
              <span>{apiKey ? "Your AI key (Active)" : "Cloud AI (Server key)"}</span>
            </button>
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                ...
                <h3 style={{ margin: 0, fontSize: "1.1rem", fontWeight: 600 }}>Your own AI key (Optional)</h3>
              </div>
              <button
                type="button"
                onClick={() => setShowKeyModal(false)}
            ...
            <p style={{ fontSize: "12.5px", color: "var(--mist)", lineHeight: 1.5, marginBottom: "1rem" }}>
              Cloud AI already works out of the box — the assistant runs on the server's own key, and a <strong>Built-in Clinical Engine</strong> answers even offline. Optionally paste a free <strong>Google Gemini API Key</strong> (<a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noreferrer" style={{ color: "var(--cyan)", textDecoration: "underline" }}>get free key</a>) or <strong>Groq API Key</strong> (<a href="https://console.groq.com/keys" target="_blank" rel="noreferrer" style={{ color: "var(--cyan)", textDecoration: "underline" }}>get free key</a>) to use your own quota. Your key stays in this browser and is only used for your requests.
            </p>
