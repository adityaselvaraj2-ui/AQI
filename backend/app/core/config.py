...
class Settings(BaseSettings):
    ...
    # Server-only. Enables the periodic Supabase archive sync; never expose it
    # to the frontend (the anon/publishable key is the browser-safe one).
    supabase_service_role_key: str = ""

    # Operator console password (invite-code management). Lives only in .env;
    # empty disables the operator endpoints entirely.
    operator_password: str = ""
    # Groq Cloud inference key for the Health Assistant chat proxy. Server-only:
    # the browser calls /api/v1/health/chat and never sees this value.
    groq_api_key: str = ""
    # Periodic archive cadence in minutes; 0 disables the background sync.
    supabase_sync_interval_minutes: float = 15.0
