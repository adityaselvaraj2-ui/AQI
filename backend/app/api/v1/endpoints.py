...
async def health_chat_proxy(request: Request, payload: HealthChatPayload) -> dict:
    ...
    """
    key = (payload.api_key or "").strip() or get_settings().groq_api_key
    ...
    # A user-supplied key that Groq rejects must not strand the user on the
    # offline engine — fall back to the server key transparently.
    ...
    # User key rejected → retry the whole chain with the server's key.
    if key_is_bad and server_key and server_key != key:
        attempts.append({
            "model": "retry",
            "success": False,
            "error": "Supplied key rejected — retrying with the server key",
            "durationMs": 0,
        })
